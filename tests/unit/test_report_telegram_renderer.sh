...
set_ok_fixture() {
    ...
    MST_SECURITY_EVENTS_REPORT_JSON="$(make_report_from_records security_events ok "$(make_record security_events ssh ssh_login_activity auth.log ok '0 failed SSH login(s), 2 accepted, 0 root login attempts since last check.' '[]')")"
...
set_security_events_warn_fixture() {
    set_ok_fixture
    MST_SECURITY_EVENTS_REPORT_JSON="$(make_report_from_records security_events warn \
        "$(make_record security_events ssh ssh_login_activity auth.log ok '0 failed SSH login(s), 2 accepted, 0 root login attempts since last check.' '[]')" \
        "$(make_record security_events sudo sudo_group_membership sudo warn '1 new sudo user(s) detected: attacker.' '[]')")"
...
[[ "${telegram_output}" == *"Security Events"* ]] || exit 1
[[ "${telegram_output}" == *"0 failed SSH login(s), 2 accepted, 0 root login attempts since last check."* ]] || exit 1
...
[[ "${digest_output}" == *"Security Events:"* ]] || exit 1
[[ "${digest_output}" == *"✅ No issues detected."* ]] || exit 1

set_security_events_warn_fixture
set +e
warn_full_output="$(capture_report --style telegram)"
set -e
[[ "${warn_full_output}" == *"1 new sudo user(s) detected: attacker."* ]] || exit 1
set +e
warn_digest_output="$(capture_report --style digest)"
set -e
[[ "${warn_digest_output}" == *"Security Events:"* ]] || exit 1
[[ "${warn_digest_output}" == *"1 new sudo user(s) detected: attacker."* ]] || exit 1
[[ "${warn_digest_output}" != *"0 failed SSH login(s), 2 accepted"* ]] || exit 1

unset MST_SECURITY_EVENTS_REPORT_JSON
set +e
absent_digest_output="$(capture_report --style digest)"
set -e
[[ "${absent_digest_output}" == *$'Security Events:\n⚪ Not available'* ]] || exit 1
set +e
absent_full_output="$(capture_report --style telegram)"
set -e
[[ "${absent_full_output}" == *$'Security Events\n⚪ Not available'* ]] || exit 1
